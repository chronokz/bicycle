            ...
            if($continue){
              ...
              if( $input && $input['error'] == UPLOAD_ERR_OK ){
                ...
                if(Koki::is_image($configuration['path_to_store'].$filename.'.'.$ext)){
                  ...
                  $upload->cards_filename = $filename.'_cards'.'.'.$ext;
                ...
                if(Koki::is_image($configuration['path_to_store'].$filename.'.'.$ext)){
                  ...
                  if(isset($configuration['resizing']['cards']))
                    WideImage::load($configuration['path_to_store'].$upload->filename)->resize($configuration['resizing']['cards']['w'], $configuration['resizing']['cards']['h'])->saveToFile($configuration['path_to_store'].$upload->cards_filename);
     ...
     public static function remove($upload_type = false, $type_id = false,$path_to_store = false){
          ...
          if($uploads){
               foreach($uploads as $upload){
                    ...
                    if($upload->cards_filename && file_exists($path_to_store.$upload->cards_filename)) @unlink($path_to_store.$upload->cards_filename);
